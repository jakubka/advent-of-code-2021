...
def add_lists_to_node(n, lists):
    if len(lists) == 0:
        ...
        debug(f"After addition and reduce: {new_node.visualise()}")
        return add_lists_to_node(new_node, lists[1:])


def add_two_lists_and_get_magni(l1, l2):
    n = add_and_reduce_lists(l1, l2)
    return n.get_magnitude()


def get_highest_magni(lists):
    max_magni = 0
    for i in range(len(lists)):
        for j in range(len(lists)):
            if i == j:
                continue
            m = add_two_lists_and_get_magni(lists[i], lists[j])
            if m > max_magni:
                max_magni = m
    return max_magni
